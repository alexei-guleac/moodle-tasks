...
import com.spring.documentale.ui.view.report.DocumentsDesignView;
import com.spring.documentale.ui.view.report.DocumentsServiceReportsView;
import com.spring.documentale.ui.view.report.DocumentsSlaView;
...
@Route(value = Routes.OPERATOR_BANK)
public class BankOperatorView extends VerticalLayout implements RouterLayout {

  private final Map<Tab, String> tabToUrlMap = new LinkedHashMap<>();

  private final SecurityService securityService;

  public BankOperatorView(@Autowired SecurityService securityService) {
    this.securityService = securityService;

    Tabs tabs = getTabs();

    UI.getCurrent().navigate(DocumentsServiceReportsView.class);
    ...
    Text space = new Text("       ");
    Text text = new Text("Operator Bank management");
  ...
  private Tabs getTabs() {
    ...
    tabToUrlMap.put(new Tab("Service Report"),
        routeConfiguration.getUrl(DocumentsServiceReportsView.class));
    tabToUrlMap.put(new Tab("Report SLA"), routeConfiguration.getUrl(DocumentsSlaView.class));
    tabToUrlMap.put(new Tab("Design"), routeConfiguration.getUrl(DocumentsDesignView.class));
